...
namespace PelShield
{
	...
	public class Gizmo_EnergyPelShieldStatus : Gizmo
	{
		...
		public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth)
		{
			...
			{
				Rect rect2;
				Rect rect4 = rect2 = GenUI.ContractedBy(GenUI.AtZero(overRect), 6f);
				rect2.height = overRect.height / 2f;
				Text.Font = 0;
				Widgets.Label(rect2, this.shield.LabelCap);
				Rect rect3 = rect4;
				rect3.yMin = overRect.height / 2f;
				float fillPercent = this.shield.energy / Mathf.Max(1f, StatExtension.GetStatValue(this.shield, StatDefOf.EnergyShieldEnergyMax, true));
				...
				Widgets.Label(rect3, (this.shield.energy * 100f).ToString("F0") + " / " + (StatExtension.GetStatValue(this.shield, StatDefOf.EnergyShieldEnergyMax, true) * 100f).ToString("F0"));
				Text.Anchor = 0;
			}, true, false, 1f);
