...
namespace PelShield
{
	...
	public class PelShieldApparel : Apparel
	{
		...
		public override void Tick()
		{
			...
			if (wearer != null && Gen.IsHashIntervalTick(wearer, 2500))
			{
		...
		public bool IsAutoRepair(Apparel apparel)
		{
			if (apparel != null && apparel.def.useHitPoints)
			{
				...
				if (apparel2 != null && apparel2.tags.Count > 0)
				{
					...
					List<string> tags = apparel3?.tags;
					if (tags.Count > 0)
					{
		...
		{
			Pawn wearer = base.Wearer;
			SoundStarter.PlayOneShot(SoundDefOf.EnergyShield_AbsorbDamage, new TargetInfo(wearer.Position, wearer.Map, false));
			this.impactAngleVect = Vector3Utility.HorizontalVectorFromAngle(dinfo.Angle);
			Vector3 loc = GenThing.TrueCenter(wearer) + Vector3Utility.RotatedBy(this.impactAngleVect, 180f) * 0.5f;
		...
		{
			if (this.ShieldState == null && this.ShouldDisplay)
			{
				Pawn wearer = base.Wearer;
